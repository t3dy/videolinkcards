"""Parse all *.txt files in this directory, extract URLs + captions, emit links.html.

The generated page supports per-card overrides (caption, tags, order) stored in
the browser's localStorage. Use Export/Import buttons in the page to back up
or move overrides between machines.
"""
import re
import html
import json
...
# Directories scanned for *.txt link lists.
# Each entry is (directory, label_prefix). Empty prefix → just the filename
# in the card's source badge. Non-empty prefix is prepended ("desktop/foo.txt").
SOURCE_DIRS = [
    (HERE, ''),
    (Path('C:/Users/PC/Desktop/p txt'), 'desktop/'),
]

# Individual .txt files to include (used for folders like Downloads where
# scanning everything would pull in unrelated files).
SOURCE_FILES = [
    (Path('C:/Users/PC/Downloads/l.txt'), 'downloads/'),
    (Path('C:/Users/PC/Downloads/p new 72425.txt'), 'downloads/'),
]
...
def iter_source_files():
    """Yield (Path, source_label) for every configured .txt file."""
    SKIP = {'requirements_recorder.txt', 'requirements.txt'}
    for d, prefix in SOURCE_DIRS:
        if not d.exists():
            continue
        for f in sorted(d.glob('*.txt')):
            if f.name in SKIP:
                continue
            yield f, f'{prefix}{f.name}'
    for f, prefix in SOURCE_FILES:
        if f.exists():
            yield f, f'{prefix}{f.name}'
...
def parse_file(path: Path, source_label: str = None):
    """Parse a single .txt file. source_label is used for the card's source badge;
    ...
    for i, line in enumerate(lines):
        ...
        for idx, p in enumerate(parts):
            if URL_RE.match(p):
                url_segments.append(p)
            else:
                if idx == 0:
                    leading_text = p.strip()

        if not url_segments:
            pending_caption = stripped
            continue

        for j, seg in enumerate(url_segments):
            m = URL_RE.match(seg)
            url = clean_url(m.group(0))
            if j == 0:
                caption = leading_text or pending_caption or None
            else:
                caption = None
            entries.append({
                'url': url,
...
def classify(url: str):
    m = re.search(r'pornhub\.com/.*viewkey=([a-zA-Z0-9]+)', url)
    if m:
        return ('pornhub', f'https://www.pornhub.com/embed/{m.group(1)}')
    m = re.search(r'xvideos\.com/video[.]?([a-z0-9]+)', url, re.IGNORECASE)
    if m:
        return ('xvideos', f'https://www.xvideos.com/embedframe/{m.group(1)}')
    m = re.search(r'xhamster\.com/videos/[^/?#]+-([a-zA-Z0-9]+)', url)
    if m:
        return ('xhamster', f'https://xhamster.com/embed/{m.group(1)}')
    m = re.search(r'eporner\.com/video-([a-zA-Z0-9]+)', url)
    if m:
        return ('eporner', f'https://www.eporner.com/embed/{m.group(1)}/')
    m = re.search(r'hdzog\.com/videos/(\d+)', url)
    if m:
        return ('hdzog', f'https://hdzog.com/embed/{m.group(1)}/')
    m = re.search(r'ok\.xxx/video/(\d+)', url)
    if m:
        return ('ok.xxx', f'https://ok.xxx/embed/{m.group(1)}/')
    m = re.search(r'beeg\.com/-?(\d+)', url)
    if m:
        return ('beeg', f'https://beeg.com/embed/{m.group(1)}')
    m = re.search(r'spankbang\.com/([a-z0-9]+)/video/', url)
    if m:
        return ('spankbang', f'https://spankbang.com/embed/{m.group(1)}')
    if 'spankbang.com/' in url and '/playlist/' in url:
        return ('spankbang', None)
    if 'megatube.xxx/' in url:
        return ('megatube', None)
    if 'pornzog.com/video/' in url:
        return ('pornzog', None)
    if 'tnaflix.com/' in url and '/video' in url:
        return ('tnaflix', None)
    dm = re.search(r'https?://([^/]+)', url)
    domain = dm.group(1) if dm else 'link'
...
# Stop-words excluded from auto-generated caption tags.
STOPWORDS = frozenset({
    'my', 'i', 'me', 'of', 'scene', 'a', 'on', 'with', 'you', 'in', 'is',
    'she', 'another', 'face', 'her', 'm', 'the', 'to', 'whore', 'all',
    'and', 'back', 'big', 'from', 's', 't', 'than', '3', 'as', 'b',
    'belongs', 'can', 'cut', 'deep', 'eye', 'for',
})


def caption_to_tags(cap: str):
    """Lowercase, strip diacritics, return alphanumeric word runs as tag list.

    Stop-words from STOPWORDS are filtered out.
    """
    if not cap:
        return []
    norm = unicodedata.normalize('NFKD', cap.lower())
    norm = ''.join(c for c in norm if not unicodedata.combining(c))
    return [w for w in WORD_RE.findall(norm) if w not in STOPWORDS]


def load_thumb_cache():
    p = HERE / '.thumb_cache.json'
    if not p.exists():
        return {}
    try:
...
def main():
    ...
    # Build per-card default tags from caption words.
    default_overrides = {}
    for c in cards:
        tags = caption_to_tags(c.get('caption') or '')
        # Dedupe preserving order
        seen_t = set()
        uniq = [t for t in tags if not (t in seen_t or seen_t.add(t))]
        if uniq:
            default_overrides[c['url']] = {'tags': uniq}
    ...
    def render_one(c, index):
        ...
        if embed:
            ...
            thumb = (
                f'<button class="thumb" type="button" {embed_attr}>'
                f'{img}'
            ...
            thumb = (
                f'<a class="thumb static" href="{url}" target="_blank" rel="noopener noreferrer">'
                f'<img class="poster" loading="lazy" '
                f'src="{html.escape(thumb_url, quote=True)}" '
                f'referrerpolicy="no-referrer" alt="">'
...
<article class="card" data-url="{url}" data-source="{src}" data-type="{html.escape(c['src_type'])}" data-default-order="{index}">
  ...
  <div class="edit-panel hidden">
    ...
    <input class="edit-tags" type="text" placeholder="e.g. favorite, redhead, anal">
    ...
    # Single global ordering: embeddable first, then lockers.
    combined = embeddable + lockers
    combined_cards_html = '\n'.join(render_one(c, i) for i, c in enumerate(combined))
...
<head>
<meta charset="utf-8">
<title>Links from text files</title>
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  .edit-move, .edit-buttons {{ display: flex; gap: 4px; align-items: center; margin-top: 4px; }}
  .edit-move button, .edit-buttons button {{ background: var(--card); color: var(--fg);
    border: 1px solid var(--card-border); border-radius: 6px; padding: 5px 10px;
    cursor: pointer; font: inherit; font-size: 12px; }}
  .edit-move button:hover, .edit-buttons button:hover {{ border-color: var(--muted); }}
  ...
  .tab {{ background: transparent; color: var(--muted); border: 1px solid var(--card-border);
    padding: 6px 14px; border-radius: 999px; cursor: pointer; font: inherit; }}
  .tab.active {{ background: var(--accent); color: #1a1d24; border-color: var(--accent); }}
  ...
  .conn {{ font-size: 11px; padding: 2px 8px; border-radius: 999px; }}
  .conn.online {{ background: #1a2a1a; color: #6dd17f; }}
  .conn.offline {{ background: #2a1a1a; color: #ff5252; }}
...
<header>
  <h1>Links</h1>
  <span class="stats">{len(cards)} unique ({len(embeddable)} embeddable · {len(lockers)} link-only) · <span id="modified-count">0</span> edited · <span id="deleted-count">0</span> deleted</span>
  ...
  <span class="conn offline" id="conn" title="Server connection status">offline</span>
  <button class="icon-btn" id="export-btn" title="Download your edits as JSON">Export</button>
  <button class="icon-btn" id="import-btn" title="Load edits from a JSON file">Import</button>
  <button class="icon-btn hidden" id="migrate-btn" title="One-time: copy edits from this browser's localStorage to the server DB">Migrate from localStorage</button>
  <button class="icon-btn" id="reset-all-btn" title="Clear all edits">Reset all</button>
...
<script>
  const STORAGE_KEY = 'links_overrides_v2';  // legacy, used only for migration
  ...
  async function loadAllFromServer() {{
    try {{
      const data = await api('/overrides');
      overrides = data || {{}};
      setConnState(true);
    }} catch (e) {{
  ...
  // Mirror what the server stores: send only fields that changed.
  async function patchOverride(url, patch) {{
    // Local update first (optimistic UI)
    const before = overrides[url] ? {{ ...overrides[url] }} : null;
    const next = {{ ...(overrides[url] || {{}}) }};
    for (const [k, v] of Object.entries(patch)) {{
      if (v === null || (k === 'tags' && Array.isArray(v) && v.length === 0 && next.tags === undefined)) {{
        delete next[k];
      }} else {{
        next[k] = v;
      }}
    }}
    if (Object.keys(next).length === 0) delete overrides[url];
    else overrides[url] = next;
    try {{
      await api('/override', {{ method: 'POST', body: JSON.stringify({{ url, ...patch }}) }});
      setConnState(true);
    }} catch (e) {{
      // Revert on failure
      if (before) overrides[url] = before; else delete overrides[url];
      setConnState(false);
  ...
  async function clearOverrideOnServer(url) {{
    const before = overrides[url] ? {{ ...overrides[url] }} : null;
    delete overrides[url];
    try {{
      await api('/override/clear', {{ method: 'POST', body: JSON.stringify({{ url }}) }});
      setConnState(true);
    }} catch (e) {{
      if (before) overrides[url] = before;
      setConnState(false);
  ...
  // Default tags derived from caption words (baked at build time).
  ...
  // Effective tags = user override if explicitly set, else defaults.
  ...
  function effectiveOrder(card) {{
    const ov = getOverride(card.dataset.url);
    return (typeof ov.order === 'number') ? ov.order : parseFloat(card.dataset.defaultOrder);
  }}
  ...
  function applyOverridesToCard(card) {{
    ...
    if (ov.caption != null && ov.caption !== '') {{
      capEl.textContent = ov.caption;
    ...
    card.classList.toggle('deleted', ov.deleted === true);
    // Highlight comment button if a comment exists
    const cmtBtn = card.querySelector('.comment-btn');
    if (cmtBtn) cmtBtn.classList.toggle('has-comment', !!(ov.comment));
  }}
  ...
  /** Compute an order value that lands this card at 1-based position `target`
   *  in the currently visible+sorted list. */
  function orderForPosition(card, target) {{
    ...
    // Sandwich between position target-1 and target (1-indexed) of `others`.
  ...
  function rebuildTagBar() {{
    ...
    for (const card of cards) {{
      const tags = effectiveTags(card.dataset.url);
  ...
  function applyFilter() {{
    ...
    for (const c of cards) {{
      ...
      // Tag filter: AND semantics — card must have all active tags
      let matchTags = true;
  ...
  function updateDeletedCount() {{
    const n = Object.values(overrides).filter(o => o && o.deleted).length;
  ...
  document.addEventListener('click', (e) => {{
    // Click on a caption → inline-edit
    ...
    if (cap && !cap.isContentEditable
      ...
      startInlineCaptionEdit(cap);
      return;
    }}

    // Click on a tag chip on a card → toggle that tag in the filter
    ...
    if (editBtn) {{
      ...
      const ov = getOverride(url);
      const capEl = card.querySelector('.caption');
      const currentCap = (ov.caption != null && ov.caption !== '')
        ? ov.caption
        : (capEl.classList.contains('muted') ? '' : capEl.textContent);
      panel.querySelector('.edit-caption').value = currentCap;
    ...
    if (actionBtn) {{
      handleEditAction(actionBtn);
      return;
    }}
  }});

  async function handleEditAction(actionBtn) {{
    ...
    if (action === 'save') {{
      ...
      await patchOverride(url, {{ caption: cap || null, tags }});
    ...
    if (action === 'reset') {{
      await clearOverrideOnServer(url);
      applyOverridesToCard(card);
      rebuildTagBar();
      applyFilter();
      applySort();
      ...
      panel.classList.add('hidden');
      card.classList.remove('editing');
      toast('Card reset to defaults.');
      return;
    }}
    if (action === 'delete') {{
      const prev = overrides[url] ? {{ ...overrides[url] }} : null;
      await patchOverride(url, {{ deleted: true }});
      applyOverridesToCard(card);
      applyFilter();
      updateModifiedCount();
      updateDeletedCount();
      toast('Card deleted.', async () => {{
        if (prev) {{
          await patchOverride(url, {{
          ...
          // restore handled by server's merge of nulls; if all empty, server prunes
          if (prev.deleted) await patchOverride(url, {{ deleted: false }});
        }} else {{
          await clearOverrideOnServer(url);
        }}
        applyOverridesToCard(card);
    ...
    if (action === 'restore') {{
      await patchOverride(url, {{ deleted: false }});
      applyOverridesToCard(card);
      applyFilter();
      updateModifiedCount();
      updateDeletedCount();
    ...
    if (action === 'move-down-10') {{
      ...
      if (targetIdx > idx) {{
        const newOrder = orderForPosition(card, targetIdx + 1);
        await patchOverride(url, {{ order: newOrder }});
        applyOverridesToCard(card);
        applySort();
        updateAllPositions();
        updateModifiedCount();
    ...
    if (action === 'goto-position') {{
      ...
      if (!Number.isInteger(target) || target < 1 || target > M) {{
        toast(`Invalid position — enter a number between 1 and ${{M}}.`);
        return;
      }}
      if (target === currentPos) return;
      const newOrder = orderForPosition(card, target);
      await patchOverride(url, {{ order: newOrder }});
      applyOverridesToCard(card);
      applySort();
      updateAllPositions();
    ...
    if (action.startsWith('move-')) {{
      ...
      if (action === 'move-up' && idx > 0) {{
        await swapOrder(card, visibleCards[idx - 1]);
      }} else if (action === 'move-down' && idx < visibleCards.length - 1) {{
        await swapOrder(card, visibleCards[idx + 1]);
      }} else if (action === 'move-top') {{
        const allOrders = visibleCards.map(effectiveOrder);
        await patchOverride(url, {{ order: Math.min(...allOrders) - 1 }});
      }} else if (action === 'move-bottom') {{
        const allOrders = visibleCards.map(effectiveOrder);
        await patchOverride(url, {{ order: Math.max(...allOrders) + 1 }});
      }}
      applyOverridesToCard(card);
      applySort();
      updateAllPositions();
  ...
  async function swapOrder(a, b) {{
    ...
    await patchOverride(a.dataset.url, {{ order: ob }});
    await patchOverride(b.dataset.url, {{ order: oa }});
    applyOverridesToCard(a);
    applyOverridesToCard(b);
  }}
  ...
  function startInlineCaptionEdit(cap) {{
    ...
    const url = card.dataset.url;
    const ov = getOverride(url);
    // Effective current text (empty if the card was displaying "(no caption)")
    const current = (ov.caption != null && ov.caption !== '')
      ? ov.caption
      : (cap.classList.contains('muted') ? '' : cap.textContent);

    cap.dataset.savedText = current;
  ...
  async function commitInlineCaption(cap, save) {{
    const card = cap.closest('.card');
    const url = card.dataset.url;
    cap.contentEditable = 'false';
    cap.classList.remove('editing-inline');
    delete cap.dataset.savedText;
    if (save) {{
      const newText = cap.textContent.trim();
      try {{
        await patchOverride(url, {{ caption: newText || null }});
        applyOverridesToCard(card);
        updateModifiedCount();
      }} catch {{
        applyOverridesToCard(card);   // revert on error
      }}
    }} else {{
      applyOverridesToCard(card);      // revert to previous
    }}
  }}
  ...
  document.getElementById('comment-save').addEventListener('click', async () => {{
    ...
    await patchOverride(commentCardUrl, {{ comment: text || null }});
  ...
  document.getElementById('comment-delete').addEventListener('click', async () => {{
    if (!commentCardUrl) return;
    await patchOverride(commentCardUrl, {{ comment: null }});
    const card = cards.find(c => c.dataset.url === commentCardUrl);
    if (card) applyOverridesToCard(card);
    updateModifiedCount();
    closeCommentModal();
  ...
  let recognizerBaseText = '';  // text already in textarea when recording started
  ...
  function getRecognizer() {{
    ...
    recognizer.onresult = (e) => {{
      ...
      // If final, bake into baseText so the next interim isn't appended twice
      if (finalText) {{
        recognizerBaseText = combined;
      }}
    }};
    ...
    recognizer.onend = () => {{
      if (recognizing) {{
        // Browser auto-stopped — restart unless we asked to stop
  ...
  document.getElementById('export-btn').addEventListener('click', () => {{
    ...
    a.download = 'links_overrides.json';
    a.click();
  ...
  document.getElementById('import-file').addEventListener('change', async (e) => {{
    ...
    try {{
      ...
      await api('/overrides/bulk', {{
        method: 'POST',
        body: JSON.stringify({{ overrides: incoming, mode: 'merge' }}),
      }});
  ...
  document.getElementById('reset-all-btn').addEventListener('click', async () => {{
    if (!confirm('Delete ALL your edits (captions, tags, order, comments)? This cannot be undone unless you exported a backup.')) return;
    try {{
      await api('/overrides/wipe', {{ method: 'POST' }});
      overrides = {{}};
      activeTags.clear();
      cards.forEach(applyOverridesToCard);
      rebuildTagBar();
  ...
  // Migrate-from-localStorage: shown only if old data exists AND server has nothing
  function checkMigrationCandidate() {{
    let legacy = null;
    try {{ legacy = JSON.parse(localStorage.getItem(STORAGE_KEY) || 'null'); }}
    catch {{}}
    if (!legacy || typeof legacy !== 'object' || !Object.keys(legacy).length) return;
    if (Object.keys(overrides).length > 0) return;   // server already has data — skip
    document.getElementById('migrate-btn').classList.remove('hidden');
  }}
  document.getElementById('migrate-btn').addEventListener('click', async () => {{
    let legacy = null;
    try {{ legacy = JSON.parse(localStorage.getItem(STORAGE_KEY) || 'null'); }}
    catch {{ toast('No legacy data found'); return; }}
    if (!legacy || !Object.keys(legacy).length) {{ toast('No legacy data found'); return; }}
    if (!confirm(`Copy ${{Object.keys(legacy).length}} legacy entries from this browser into the server DB?`)) return;
    try {{
      await api('/overrides/bulk', {{ method: 'POST',
        body: JSON.stringify({{ overrides: legacy, mode: 'merge' }}) }});
      await loadAllFromServer();
      cards.forEach(applyOverridesToCard);
      rebuildTagBar();
      applySort();
      ...
      updateModifiedCount();
      updateDeletedCount();
      document.getElementById('migrate-btn').classList.add('hidden');
      toast(`Migrated ${{Object.keys(legacy).length}} entries. Local storage left intact as a backup.`);
    }} catch (e) {{
  ...
  // Periodic health check: flip the badge to "online" automatically when
  // the server comes back, and warn early if it goes down.
  setInterval(async () => {{
    try {{
      ...
      setConnState(r.ok);
      if (wasOffline && r.ok) {{
        // Re-sync: server may have new data we don't know about.
        await loadAllFromServer();
        cards.forEach(applyOverridesToCard);
        rebuildTagBar();
        applySort();
...
'''
    ...
    print(f'Wrote {out} - {len(cards)} unique links')
    print(f'  Embeddable: {len(embeddable)}, Lockers: {len(lockers)}')
